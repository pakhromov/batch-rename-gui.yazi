...
Yazi's built-in bulk rename only works with terminal editors, this plugin allows using batch rename in a GUI text editor like VS Code or Sublime Text. Single file or no selection uses Yazi's built-in inline rename. When multiple files are selected, a configured GUI editor is opened with one filename per line and renaming happens the moment you save the opened file.
